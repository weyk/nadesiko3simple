...
import { Ast, FuncList, FuncArgs, Token, NakoDebugOption } from 'nadesiko3core/src/nako_types.mjs'
import { NakoCompiler } from 'nadesiko3core/src/nako3.mjs'
import { NakoGen, NakoGenOptions, NakoGenResult } from 'nadesiko3core/src/nako_gen.mjs'
...
export class NakoGenSimple {
  ...
  */
   ...
   * @param isTest
   */
  getDefFuncCode (com: NakoCompiler, isTest: boolean): string {
    let code = ''
    // よく使う変数のショートカット
  ...
  convFor (node: Ast): string {
    ...
    const loopDir = node.loopDirection
    let inc = '1'
    ...
    const code =
      ...
      '  };\n' +
      `};\n//[/FOR id=${idLoop}]\n`
    return this.convLineno(node, false) + loopDir === null ? code : loopDir === 'up' ? codeUp : codeDown
  }
  ...
  convCallFunc (node: Ast, isExpression: boolean): string {
    const unwrapUserfunc = (js:string) => {
      const check = /__varslist\[1\]\["([^"]*)"\]/.exec(js)
      return check === null ? js : check[1]
    }
    ...
    let funcCall = `${unwrapUserfunc(res.js)}(${argsCode})`
    if (func.asyncFn) {
    ...
    if (func.return_none) {
      ...
      // 戻り値のある関数の場合
      let sorePrefex = ''
      if (this.speedMode.invalidSore === 0 && (node.josi === 'して' || (node.josi === '' && !isExpression))) {
        sorePrefex = `${this.varname('それ')} = `
      }
   ...
   * @param isTest
   */
  static generate (com: NakoCompiler, ast: Ast, opt: NakoGenOptions|boolean): NakoGenResult {
    // NakoGenのインスタンスを作成
    const gen = new NakoGenSimple(com)

    if (typeof opt === 'boolean') {
      opt = new NakoGenOptions(opt)
    }
    ...
    // (2) JSコードを生成する
    let js = gen.convGen(ast, opt.isTest)

    // (3) JSコードを実行するための事前ヘッダ部分の生成
    const jsInit = gen.getDefFuncCode(com, opt.isTest)
...
`
    ...
    // Standalone用のpluginをImportするコードを生成
    let codeImportFiles = ''
    const importNames = []
...
`
  ...
  static selfRegister (nako3: NakoCompiler) {
    nako3.addCodeGenerator2('simple', NakoGenSimple)
    console.log(`regist simple generator`)
  }
}
...
if (typeof (navigator) === 'object' && typeof ((navigator as any).nako3) === 'object') {
  ...
  if (nako3.addCodeGenerator2) { nako3.addCodeGenerator2('simple', NakoGenSimple) }
  else
  if (nako3.addCodeGenerator) { nako3.addCodeGenerator('simple', NakoGenSimple) }
}

export default NakoGenSimple
